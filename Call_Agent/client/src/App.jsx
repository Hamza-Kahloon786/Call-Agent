...
function App() {
  ...
  useEffect(() => {
    ...
    // ✅ CHANGED: Redirect to LOGIN instead of order status
    if (paymentStatus === 'success' && sessionId && orderId) {
      toast({
        title: "Payment Successful! 🎉",
        description: "Please login to view your order status.",
        duration: 3000,
      });
      
      // Get email from context or use a default
      const email = localStorage.getItem('checkoutEmail') || '';
      
      // Redirect to login with order info
      setTimeout(() => {
        navigate(`/login?payment=success&orderId=${orderId}&email=${encodeURIComponent(email)}`);
      }, 1500);
  ...
  const STRIPE_PUBLISHABLE_KEY = import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY || 'pk_test_51S5wBRLtHdMzEuoPpqhJ5VrJLtLxjJunlmRHIcguRJ2Z6rjNjg5sPpobEhq6hIUinbBT6vwwSWHQZU88JHgiIlgN00csuuUUzs';
